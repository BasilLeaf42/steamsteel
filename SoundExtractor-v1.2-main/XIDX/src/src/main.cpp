...
int main(int argc, char *argv[]){
    ...
    fputs( "idx extractor/packer\nVersion 1.2\nMade by Vercingetorix, updated to build on Linux/GCC by The Europa Barbarorum Team\n\n", stderr );
	...
	while( *pstr ){
	    switch ( *pstr ){
		case 'm':
		    if (function == 0 || function == CREATE) {
			medieval2 = true;
		    }
		    else {
			fprintf(stderr, "The '-m' flag does not affect %s packs.\n", function == EXTRACT ? "extracting" : "listing");
		    }
		    break;
		case 'b':
		    if( perform_suffix_manip) {
			fputs( "Ignoring redunant -b flag.\n", stderr);
		    }
		    perform_suffix_manip = true;
		    break;
		case 'a':
		...
		case 's':
		    ...
		    break;                  
		case 'x':
		    code = func_type_check(function, EXTRACT);
		    if ( medieval2 && code == 0) {
			fputs( "The '-m' flag does not affect extracting packs.\n", stderr);
		    }
		    break;
		case 'c':
		    ...
		    break;
		case 't':
		    code = func_type_check(function, LIST);
		    if ( medieval2 && code == 0) {
			fputs( "The '-m' flag does not affect listing packs.\n", stderr);
		    }
		    break;
		case 'v':
		    if( verbose ) {
			fputs( "Ignoring redunant -v flag.\n", stderr);
		    }
		    verbose = true;
		    break;
		case 'f':
	...
	case IDXSEVT:
	    if( medieval2 ) {
		fputs( "The '-m' flag does not affect event packs." , stderr);
	    }
	    idx = new eidxFile;
	    break;
	default:
	...
	case CREATE:
	    ...
	    fprintf( stderr, "\nCreating idx file...\n" );               
	    if (idx->Write( ofile, files.GetData(), files.GetSize()) ) {
		fprintf(stderr, "Successfully created pack: '%s'\n", ofile);
	    }
	    else {
