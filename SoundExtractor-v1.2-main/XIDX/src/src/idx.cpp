...
void replace_suffix(const char* filename, const char* suffix, int len,char* r){
    return append_suffix(filename, suffix, len -4, r);
}

int basename_length(char* name, int len) {
...
void mangle_name(char* name, char* r, int len) {
    ...
    if (len2 > 4) {
	char *suffix = name + (len -4) * sizeof(char);
	if (strcasecmp(suffix, ".bin") == 0) {
	    strncpy(r, name, len);
	    return;
	}
    }
    if(len2 > 250) {
	replace_suffix(name, ".bin", len, r);
    }
    else {
	append_suffix(name, ".bin", len, r);
    }
}
...
bool idxFile::OpenFiles( const char* filename, const char* mode ){
    ...
    if(len > 4) {
	const char* suffix = filename + (len-4) * sizeof(char);
	if ( strcasecmp(suffix, ".dat") == 0) {
	   
	    replace_suffix(filename, ".idx", len, p1);
	    strcpy(p2, filename);
	}
	else {
	    if(strcasecmp(suffix, ".idx")==0) {
		strcpy(p1,filename);
		replace_suffix(filename, ".dat", len, p2);
	    }
	    else {
		append_suffix(filename, ".idx", len, p1);
		append_suffix(filename, ".dat", len, p2);
	    }
	}
    }
    else {
...
bool idxFile::Write( const char* filename, const char** files, int count ){
    ...
    for( int i = 0; i<count; i++ ){
        ...
        if( !result ) fprintf( stderr, "failed to pack file %s\n", subFiles->filename);
        else if( verbose ) {
	    if(perform_suffix_manip) {
		int ll =strlen(filename);
		char fn[ll+4];
...
bool aidxSubFile::Write( FILE* fp ){
    ...
    IO_CHECK(fwrite(&entrySize, sizeof(int), 1, fp), 1)
    IO_CHECK(fwrite(&beginFileOffset, sizeof(int), 1, fp), 1)
    IO_CHECK(fwrite(&fileSize, sizeof(int), 1, fp), 1)
